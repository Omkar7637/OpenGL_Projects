...
@implementation GLESView
{
    ...
    // Texture Variables
    GLuint texture_checkboard_AMC;
    GLubyte checkImage_AMC[CHECK_IMAGE_HIGHT][CHECK_IMAGE_WIDTH][4];
}
...
-(int)initialize
{
    ...
    const GLfloat square_texcoord_AMC[] =
    {
        1.0f,  1.0f,
        0.0f,  1.0f,
        0.0f,  0.0f,
    ...
    glGenBuffers(1, &vbo_texcoord_AMC);
    glBindBuffer(GL_ARRAY_BUFFER, vbo_texcoord_AMC);
    glBufferData(GL_ARRAY_BUFFER, sizeof(square_texcoord_AMC), square_texcoord_AMC, GL_STATIC_DRAW);
    glVertexAttribPointer(
        AMC_ATTRIBUTE_TEXCOORD,
        2,
        GL_FLOAT,
...
-(void)makeCheckImage
{
    ...
    int i_AMC, j_AMC, c_AMC;

    for (i_AMC = 0; i_AMC < CHECK_IMAGE_HIGHT; i_AMC++)
    {
        for (j_AMC = 0; j_AMC < CHECK_IMAGE_WIDTH; j_AMC++)
        {
            c_AMC = (((i_AMC & 0x8) == 0) ^ ((j_AMC & 0x8) == 0)) * 255;

            checkImage_AMC[i_AMC][j_AMC][0] = (GLubyte)c_AMC;
            checkImage_AMC[i_AMC][j_AMC][1] = (GLubyte)c_AMC;
            checkImage_AMC[i_AMC][j_AMC][2] = (GLubyte)c_AMC;
            checkImage_AMC[i_AMC][j_AMC][3] = (GLubyte)255;
        }
    }
}
...
-(void)loadGLTexture
{
    ...
    glGenTextures(1, &texture_checkboard_AMC);
    glBindTexture(GL_TEXTURE_2D, texture_checkboard_AMC);
    glPixelStorei(GL_UNPACK_ALIGNMENT, 1);
    glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_S, GL_REPEAT);
    glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_T, GL_REPEAT);
    glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_NEAREST);
    glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_NEAREST);
    glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, CHECK_IMAGE_WIDTH, CHECK_IMAGE_HIGHT, 0, GL_RGBA, GL_UNSIGNED_BYTE, (void*)checkImage_AMC);
...
{
    // ------------------------------------------- VARIABLE DECLARATIONS -----------------------------------------------------------------
    GLfloat square_positions_AMC[12];
    ...
    // Translate
    modelViewMatrix_AMC = vmath::translate(0.0f, 0.0f, -4.0f);
    ...
    square_positions_AMC[0] = 0.0f;
    square_positions_AMC[1] = 1.0f;
    square_positions_AMC[2] = 0.0f;

    square_positions_AMC[3] = -2.0f;
    square_positions_AMC[4] = 1.0f;
    square_positions_AMC[5] = 0.0f;
    
    square_positions_AMC[6] = -2.0f;
    square_positions_AMC[7] = -1.0f;
    square_positions_AMC[8] = 0.0f;
    
    square_positions_AMC[9] = 0.0f;
    square_positions_AMC[10] = -1.0f;
    square_positions_AMC[11] = 0.0f;

    glBindBuffer(GL_ARRAY_BUFFER, vbo_position_AMC);
    glBufferData(GL_ARRAY_BUFFER, sizeof(square_positions_AMC), square_positions_AMC, GL_DYNAMIC_DRAW);
    glBindBuffer(GL_ARRAY_BUFFER, 0);
    ...
    square_positions_AMC[0] = 2.41421f;
    square_positions_AMC[1] = 1.0f;
    square_positions_AMC[2] = -1.41421f;

    square_positions_AMC[3] = 1.0f;
    square_positions_AMC[4] = 1.0f;
    square_positions_AMC[5] = 0.0f;

    square_positions_AMC[6] = 1.0f;
    square_positions_AMC[7] = -1.0f;
    square_positions_AMC[8] = 0.0f;

    square_positions_AMC[9] = 2.41421f;
    square_positions_AMC[10] = -1.0f;
    square_positions_AMC[11] = -1.41421f;
...
-(void)uninitialize
{
    ...
    if (vbo_texcoord_AMC)
    {
        glDeleteBuffers(1, &vbo_texcoord_AMC);
        vbo_texcoord_AMC = 0;
    }
