...
@implementation GLESView
{
    ...
    mat4 orthographicProjectionMatrix_SAG;
}
...
-(int)initialize
{
    ...
    orthographicProjectionMatrix_SAG = vmath::mat4::identity();
...
-(void)resize:(int)width_SAG :(int)height_SAG
{
    ...
    if (width_SAG <= height_SAG)
    {
        orthographicProjectionMatrix_SAG = vmath::ortho(
            -100.0f,
             100.0f,
            -100.0f * ((GLfloat)height_SAG / (GLfloat)width_SAG),
             100.0f * ((GLfloat)height_SAG / (GLfloat)width_SAG),
            -100.0f,
             100.0f
        );
    ...
    {
        orthographicProjectionMatrix_SAG = vmath::ortho(
            -100.0f * ((GLfloat)width_SAG / (GLfloat)height_SAG),
             100.0f * ((GLfloat)width_SAG / (GLfloat)height_SAG),
            -100.0f,
             100.0f,
            -100.0f,
...
-(void)display
{
    ...
    // Transformation
    mat4 modelViewMtrix_SAG = vmath::mat4::identity();
    mat4 modelViewProjectionMatrix_SAG = orthographicProjectionMatrix_SAG * modelViewMtrix_SAG; // here order of mult is VVIMP.
