...
GLfloat lightDiffuse_SAG[4] = { 1.0f, 1.0f, 1.0f, 1.0f };
GLfloat materialDiffuse_SAG[4] = { 0.5f, 0.5f, 0.5f, 1.0f };
GLfloat lightPosition_SAG[4] = { 0.0f, 0.0f, 2.0f, 1.0f };
...
@implementation GLESView
{
    ...
    GLuint vbo_normals_cube_SAG;

    // GLuint mvpMatrixUniform_SAG;    // This will get turn into below two
...
-(int)initialize
{
    ...
    glGenBuffers(1, &vbo_normals_cube_SAG);
    glBindBuffer(GL_ARRAY_BUFFER, vbo_normals_cube_SAG);
    glBufferData(GL_ARRAY_BUFFER, sizeof(cube_normal_SAG), cube_normal_SAG, GL_STATIC_DRAW);
    glVertexAttribPointer(
        AMC_ATTRIBUTE_NORMAL,
        3,
        GL_FLOAT,
...
-(void)uninitialize
{
    ...
    if (vbo_normals_cube_SAG)
    {
        glDeleteBuffers(1, &vbo_normals_cube_SAG);
        vbo_normals_cube_SAG = 0;
    }
