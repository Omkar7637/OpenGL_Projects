...
@implementation GLESView
{
    ...
    GLuint vao_shoulder_AMC;
    GLuint vbo_shoulder_position_AMC;
    GLuint vbo_shoulder_element_AMC;

    // ======================= VAO & VBOs For Elbow =====================
    GLuint vao_elbow_AMC;
    GLuint vbo_elbow_position_AMC;
    GLuint vbo_elbow_element_AMC;

    // ======================= VAO & VBOs For Wrist =====================
    GLuint vao_wrist_AMC;
    GLuint vbo_wrist_position_AMC;
    GLuint vbo_wrist_element_AMC;
...
-(int)initialize
{
    ...
    glGenVertexArrays(1, &vao_shoulder_AMC);
    glBindVertexArray(vao_shoulder_AMC);

    // VBO (Vertex Buffer Object) For Shoulder Position
    glGenBuffers(1, &vbo_shoulder_position_AMC);
    glBindBuffer(GL_ARRAY_BUFFER, vbo_shoulder_position_AMC);
    ...
    // element vbo
    glGenBuffers(1, &vbo_shoulder_element_AMC);
    glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, vbo_shoulder_element_AMC);
    ...
    // VAO (Vertex Array Object)
    glGenVertexArrays(1, &vao_elbow_AMC);
    glBindVertexArray(vao_elbow_AMC);

    // VBO (Vertex Buffer Object) For Elbow Position
    glGenBuffers(1, &vbo_elbow_position_AMC);
    glBindBuffer(GL_ARRAY_BUFFER, vbo_elbow_position_AMC);
    glBufferData(GL_ARRAY_BUFFER, sizeof(sphere_vertices_AMC), sphere_vertices_AMC, GL_STATIC_DRAW);
    ...
    // element vbo
    glGenBuffers(1, &vbo_elbow_element_AMC);
    glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, vbo_elbow_element_AMC);
    glBufferData(GL_ELEMENT_ARRAY_BUFFER, sizeof(sphere_elements_AMC), sphere_elements_AMC, GL_STATIC_DRAW);
    glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, 0);
    ...
    // VAO (Vertex Array Object)
    glGenVertexArrays(1, &vao_wrist_AMC);
    glBindVertexArray(vao_wrist_AMC);

    // VBO (Vertex Buffer Object) For Wrist Position
    glGenBuffers(1, &vbo_wrist_position_AMC);
    glBindBuffer(GL_ARRAY_BUFFER, vbo_wrist_position_AMC);
    glBufferData(GL_ARRAY_BUFFER, sizeof(sphere_vertices_AMC), sphere_vertices_AMC, GL_STATIC_DRAW);
    ...
    // element vbo
    glGenBuffers(1, &vbo_wrist_element_AMC);
    glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, vbo_wrist_element_AMC);
    glBufferData(GL_ELEMENT_ARRAY_BUFFER, sizeof(sphere_elements_AMC), sphere_elements_AMC, GL_STATIC_DRAW);
    glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, 0);
...
-(void)display
{
    ...
    translationMatrix_AMC = vmath::translate(0.0f, 0.0f, -10.0f);

    matrix_stack_AMC.push(translationMatrix_AMC);

    // Translate
    mat4 rotationMatrix_AMC = mat4::identity();
    rotationMatrix_AMC = vmath::rotate((GLfloat)shoulder_AMC, 0.0f, 0.0f, 1.0f);

    translationMatrix_AMC = mat4::identity();
    translationMatrix_AMC = vmath::translate(1.0f, 0.0f, 0.0f);

    modelViewMatrix_AMC = matrix_stack_AMC.top() * rotationMatrix_AMC * translationMatrix_AMC;
    matrix_stack_AMC.push(modelViewMatrix_AMC);

    mat4 scaleMatrix_AMC = mat4::identity();
    scaleMatrix_AMC = vmath::scale(2.0f, 0.5f, 1.0f);

    modelViewMatrix_AMC = modelViewMatrix_AMC * scaleMatrix_AMC;
    ...
    rotationMatrix_AMC = vmath::rotate((GLfloat)elbow_AMC, 0.0f, 0.0f, 1.0f);

    modelViewMatrix_AMC = rotationMatrix_AMC * translationMatrix_AMC;

    mat4 translationMatrix1_AMC = mat4::identity();
    translationMatrix1_AMC = vmath::translate(1.0f, 0.0f, 0.0f);

    modelViewMatrix_AMC = matrix_stack_AMC.top() * translationMatrix1_AMC * modelViewMatrix_AMC;
    matrix_stack_AMC.push(modelViewMatrix_AMC);
    ...
    rotationMatrix_AMC = mat4::identity();
    rotationMatrix_AMC = vmath::rotate((GLfloat)wrist_AMC, 0.0f, 0.0f, 1.0f);

    modelViewMatrix_AMC = translationMatrix_AMC * rotationMatrix_AMC;

    translationMatrix1_AMC = mat4::identity();
    translationMatrix1_AMC = vmath::translate(0.2f, 0.0f, 0.0f);

    modelViewMatrix_AMC = matrix_stack_AMC.top() * modelViewMatrix_AMC * translationMatrix1_AMC;
    matrix_stack_AMC.push(modelViewMatrix_AMC);

    scaleMatrix_AMC = mat4::identity();
    scaleMatrix_AMC = vmath::scale(0.4f, 0.3f, 1.0f);
...
-(void)update
{
    ...
    if(startRotation_sud)
    {
        if(rotateClockWise_sud == FALSE)
        {
            ...
            {
                shoulder_AMC = (shoulder_AMC + 3) % 360;
            }
            else if(changeFocus_AMC == 2)
            {
                elbow_AMC = (elbow_AMC + 3) % 360;
            }
            else if(changeFocus_AMC == 3)
            {
                wrist_AMC = (wrist_AMC + 3) % 360;
            }
        }
        else
        {
...
-(void)uninitialize
{
    ...
    if (vbo_shoulder_position_AMC)
    {
        glDeleteBuffers(1, &vbo_shoulder_position_AMC);
        vbo_shoulder_position_AMC = 0;
    }

    // Delete VBO (Vertex Buffer Object) for Element
    if (vbo_shoulder_element_AMC)
    {
        glDeleteBuffers(1, &vbo_shoulder_element_AMC);
        vbo_shoulder_element_AMC = 0;
    }

    // Delete VAO (Vertex Array Object)
    if (vao_shoulder_AMC)
    {
        glDeleteVertexArrays(1, &vao_shoulder_AMC);
        vao_shoulder_AMC = 0;
    }
    ...
    // Delete VBO (Vertex Buffer Object) for position
    if (vbo_elbow_position_AMC)
    {
        glDeleteBuffers(1, &vbo_elbow_position_AMC);
        vbo_elbow_position_AMC = 0;
    }

    // Delete VBO (Vertex Buffer Object) for Element
    if (vbo_elbow_element_AMC)
    {
        glDeleteBuffers(1, &vbo_elbow_element_AMC);
        vbo_elbow_element_AMC = 0;
    }

    // Delete VAO (Vertex Array Object)
    if (vao_elbow_AMC)
    {
        glDeleteVertexArrays(1, &vao_elbow_AMC);
        vao_elbow_AMC = 0;
    }
    ...
    // Delete VBO (Vertex Buffer Object) for position
    if (vbo_wrist_position_AMC)
    {
        glDeleteBuffers(1, &vbo_wrist_position_AMC);
        vbo_wrist_position_AMC = 0;
    }

    // Delete VBO (Vertex Buffer Object) for Element
    if (vbo_wrist_element_AMC)
    {
        glDeleteBuffers(1, &vbo_wrist_element_AMC);
        vbo_wrist_element_AMC = 0;
    }

    // Delete VAO (Vertex Array Object)
    if (vao_wrist_AMC)
    {
        glDeleteVertexArrays(1, &vao_wrist_AMC);
        vao_wrist_AMC = 0;
    }
