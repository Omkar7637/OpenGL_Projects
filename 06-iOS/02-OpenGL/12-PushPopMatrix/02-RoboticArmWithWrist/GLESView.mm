...
@implementation GLESView
{
    ...
    // ======================= VAO & VBOs For Shoulder =====================
    GLuint vao_shoulder_SAG;
    GLuint vbo_shoulder_position_SAG;
    GLuint vbo_shoulder_element_SAG;

    // ======================= VAO & VBOs For Elbow =====================
    GLuint vao_elbow_SAG;
    GLuint vbo_elbow_position_SAG;
    GLuint vbo_elbow_element_SAG;

    // ======================= VAO & VBOs For Wrist =====================
    GLuint vao_wrist_SAG;
    GLuint vbo_wrist_position_SAG;
    GLuint vbo_wrist_element_SAG;
...
-(int)initialize
{
    ...
    // ===================== Shoulder VAO, VBO & Elements ==============================

    // VAO (Vertex Array Object)
    glGenVertexArrays(1, &vao_shoulder_SAG);
    glBindVertexArray(vao_shoulder_SAG);

    // VBO (Vertex Buffer Object) For Shoulder Position
    glGenBuffers(1, &vbo_shoulder_position_SAG);
    glBindBuffer(GL_ARRAY_BUFFER, vbo_shoulder_position_SAG);
    ...
    // element vbo
    glGenBuffers(1, &vbo_shoulder_element_SAG);
    glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, vbo_shoulder_element_SAG);
    glBufferData(GL_ELEMENT_ARRAY_BUFFER, sizeof(sphere_elements_SAG), sphere_elements_SAG, GL_STATIC_DRAW);
    glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, 0);

    glBindVertexArray(0);

    // ===================== Elbow VAO, VBO & Elements ==============================

    // VAO (Vertex Array Object)
    glGenVertexArrays(1, &vao_elbow_SAG);
    glBindVertexArray(vao_elbow_SAG);

    // VBO (Vertex Buffer Object) For Elbow Position
    glGenBuffers(1, &vbo_elbow_position_SAG);
    glBindBuffer(GL_ARRAY_BUFFER, vbo_elbow_position_SAG);
    glBufferData(GL_ARRAY_BUFFER, sizeof(sphere_vertices_SAG), sphere_vertices_SAG, GL_STATIC_DRAW);
    ...
    // element vbo
    glGenBuffers(1, &vbo_elbow_element_SAG);
    glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, vbo_elbow_element_SAG);
    glBufferData(GL_ELEMENT_ARRAY_BUFFER, sizeof(sphere_elements_SAG), sphere_elements_SAG, GL_STATIC_DRAW);
    glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, 0);

    glBindVertexArray(0);

    // ===================== Wrist VAO, VBO & Elements ==============================

    // VAO (Vertex Array Object)
    glGenVertexArrays(1, &vao_wrist_SAG);
    glBindVertexArray(vao_wrist_SAG);

    // VBO (Vertex Buffer Object) For Wrist Position
    glGenBuffers(1, &vbo_wrist_position_SAG);
    glBindBuffer(GL_ARRAY_BUFFER, vbo_wrist_position_SAG);
    glBufferData(GL_ARRAY_BUFFER, sizeof(sphere_vertices_SAG), sphere_vertices_SAG, GL_STATIC_DRAW);
    ...
    // element vbo
    glGenBuffers(1, &vbo_wrist_element_SAG);
    glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, vbo_wrist_element_SAG);
    glBufferData(GL_ELEMENT_ARRAY_BUFFER, sizeof(sphere_elements_SAG), sphere_elements_SAG, GL_STATIC_DRAW);
    glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, 0);
...
-(void)display
{
    ...
    translationMatrix_SAG = vmath::translate(0.0f, 0.0f, -10.0f);

    matrix_stack_SAG.push(translationMatrix_SAG);

    // Translate
    mat4 rotationMatrix_SAG = mat4::identity();
    rotationMatrix_SAG = vmath::rotate((GLfloat)shoulder_SAG, 0.0f, 0.0f, 1.0f);

    translationMatrix_SAG = mat4::identity();
    translationMatrix_SAG = vmath::translate(1.0f, 0.0f, 0.0f);

    modelViewMatrix_SAG = matrix_stack_SAG.top() * rotationMatrix_SAG * translationMatrix_SAG;
    matrix_stack_SAG.push(modelViewMatrix_SAG);

    mat4 scaleMatrix_SAG = mat4::identity();
    scaleMatrix_SAG = vmath::scale(2.0f, 0.5f, 1.0f);

    modelViewMatrix_SAG = modelViewMatrix_SAG * scaleMatrix_SAG;
    ...
    glBindVertexArray(vao_shoulder_SAG);
    glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, vbo_shoulder_element_SAG);
    glVertexAttrib3f(AMC_ATTRIBUTE_COLOR, 0.8f, 0.6f, 0.4f);
    ...
    rotationMatrix_SAG = vmath::rotate((GLfloat)elbow_SAG, 0.0f, 0.0f, 1.0f);

    modelViewMatrix_SAG = rotationMatrix_SAG * translationMatrix_SAG;

    mat4 translationMatrix1_SAG = mat4::identity();
    translationMatrix1_SAG = vmath::translate(1.0f, 0.0f, 0.0f);

    modelViewMatrix_SAG = matrix_stack_SAG.top() * translationMatrix1_SAG * modelViewMatrix_SAG;
    matrix_stack_SAG.push(modelViewMatrix_SAG);
    ...
    glBindVertexArray(vao_elbow_SAG);
    glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, vbo_elbow_element_SAG);
    glVertexAttrib3f(AMC_ATTRIBUTE_COLOR, 0.75f, 0.55f, 0.35f);
    glDrawElements(GL_TRIANGLES, gNumElements_SAG, GL_UNSIGNED_SHORT, 0);
    glBindVertexArray(0);
    ...
    rotationMatrix_SAG = mat4::identity();
    rotationMatrix_SAG = vmath::rotate((GLfloat)wrist_SAG, 0.0f, 0.0f, 1.0f);

    modelViewMatrix_SAG = translationMatrix_SAG * rotationMatrix_SAG;

    translationMatrix1_SAG = mat4::identity();
    translationMatrix1_SAG = vmath::translate(0.2f, 0.0f, 0.0f);

    modelViewMatrix_SAG = matrix_stack_SAG.top() * modelViewMatrix_SAG * translationMatrix1_SAG;
    matrix_stack_SAG.push(modelViewMatrix_SAG);

    scaleMatrix_SAG = mat4::identity();
    scaleMatrix_SAG = vmath::scale(0.4f, 0.3f, 1.0f);
...
-(void)update
{
    ...
    if(startRotation_sud)
    {
        if(rotateClockWise_sud == FALSE)
        {
            if(changeFocus_SAG == 1)
            {
                shoulder_SAG = (shoulder_SAG + 3) % 360;
            }
            else if(changeFocus_SAG == 2)
            {
                elbow_SAG = (elbow_SAG + 3) % 360;
            }
            else if(changeFocus_SAG == 3)
            {
                wrist_SAG = (wrist_SAG + 3) % 360;
            }
        }
        else
        {
...
-(void)uninitialize
{
    ...
    if (vbo_shoulder_position_SAG)
    {
        glDeleteBuffers(1, &vbo_shoulder_position_SAG);
        vbo_shoulder_position_SAG = 0;
    }

    // Delete VBO (Vertex Buffer Object) for Element
    if (vbo_shoulder_element_SAG)
    {
        glDeleteBuffers(1, &vbo_shoulder_element_SAG);
        vbo_shoulder_element_SAG = 0;
    }

    // Delete VAO (Vertex Array Object)
    if (vao_shoulder_SAG)
    {
        glDeleteVertexArrays(1, &vao_shoulder_SAG);
        vao_shoulder_SAG = 0;
    }
    ...
    // Delete VBO (Vertex Buffer Object) for position
    if (vbo_elbow_position_SAG)
    {
        glDeleteBuffers(1, &vbo_elbow_position_SAG);
        vbo_elbow_position_SAG = 0;
    }

    // Delete VBO (Vertex Buffer Object) for Element
    if (vbo_elbow_element_SAG)
    {
        glDeleteBuffers(1, &vbo_elbow_element_SAG);
        vbo_elbow_element_SAG = 0;
    }

    // Delete VAO (Vertex Array Object)
    if (vao_elbow_SAG)
    {
        glDeleteVertexArrays(1, &vao_elbow_SAG);
        vao_elbow_SAG = 0;
    }
    ...
    // Delete VBO (Vertex Buffer Object) for position
    if (vbo_wrist_position_SAG)
    {
        glDeleteBuffers(1, &vbo_wrist_position_SAG);
        vbo_wrist_position_SAG = 0;
    }

    // Delete VBO (Vertex Buffer Object) for Element
    if (vbo_wrist_element_SAG)
    {
        glDeleteBuffers(1, &vbo_wrist_element_SAG);
        vbo_wrist_element_SAG = 0;
    }

    // Delete VAO (Vertex Array Object)
    if (vao_wrist_SAG)
    {
        glDeleteVertexArrays(1, &vao_wrist_SAG);
        vao_wrist_SAG = 0;
    }
...
-(void)onDoubleTap:(UITapGestureRecognizer*)gestureRecognizer
{
    ...
    if(rotateClockWise_sud == TRUE)
    {
        rotateClockWise_sud = FALSE;
    }
    else
    {
        rotateClockWise_sud = TRUE;
    }
}
...
-(void)onLongPress:(UILongPressGestureRecognizer*)gestureRecognizer
{
    ...
    if(startRotation_sud == FALSE)
    {
        startRotation_sud = TRUE;
    }
    else
    {
        startRotation_sud = FALSE;
