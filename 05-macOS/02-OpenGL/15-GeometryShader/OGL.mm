...
-(int)initialize
{
	...
	// =============================================== GEOMETRY SHADER ===============================================
	const GLchar* geometryShaderSourceCode_AMC =
		"#version 410 core																		"\
		"\n																						"\
		"layout(triangles) in;																	"\
	...
	GLuint geometryShaderObject_AMC = glCreateShader(GL_GEOMETRY_SHADER);

	glShaderSource(geometryShaderObject_AMC, 1, (const GLchar**)&geometryShaderSourceCode_AMC, NULL);

	glCompileShader(geometryShaderObject_AMC);

	status_AMC = 0;
	infoLogLength_AMC = 0;
	szInfoLog_AMC = NULL;

	glGetShaderiv(geometryShaderObject_AMC, GL_COMPILE_STATUS, &status_AMC);

	if (status_AMC == GL_FALSE)
	{
		glGetShaderiv(geometryShaderObject_AMC, GL_INFO_LOG_LENGTH, &infoLogLength_AMC); // GL_SHADER_INFO_LOG_LENGTH
		if (infoLogLength_AMC > 0)
		{
			szInfoLog_AMC = (GLchar*)malloc(infoLogLength_AMC);
			if (szInfoLog_AMC != NULL)
			{
				glGetShaderInfoLog(geometryShaderObject_AMC, infoLogLength_AMC, NULL, szInfoLog_AMC);
				fprintf(gpFile_AMC, "Geometry Shader Compilation Error LOG : %s\n", szInfoLog_AMC);
				free(szInfoLog_AMC);
				szInfoLog_AMC = NULL;
			}
		}
    ...
    glAttachShader(shaderProgramObject_AMC, geometryShaderObject_AMC);
