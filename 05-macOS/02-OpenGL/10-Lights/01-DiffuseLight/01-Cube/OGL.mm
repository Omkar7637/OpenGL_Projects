...
GLfloat lightDiffuse_AMC[4] = { 1.0f, 1.0f, 1.0f, 1.0f };
GLfloat materialDiffuse_AMC[4] = { 0.5f, 0.5f, 0.5f, 1.0f };
GLfloat lightPosition_AMC[4] = { 0.0f, 0.0f, 2.0f, 1.0f };
...
@implementation GLView
{
	...
	GLuint vbo_normals_cube_AMC;

    // GLuint mvpMatrixUniform_AMC;	// This will get turn into below two
...
-(int)initialize
{
	...
	glGenBuffers(1, &vbo_normals_cube_AMC);
	glBindBuffer(GL_ARRAY_BUFFER, vbo_normals_cube_AMC);
	glBufferData(GL_ARRAY_BUFFER, sizeof(cube_normal_AMC), cube_normal_AMC, GL_STATIC_DRAW);
	glVertexAttribPointer(
		AMC_ATTRIBUTE_NORMAL,
		3,
		GL_FLOAT,
...
-(void)display
{
	...
	//{
	//	glUniform1i(keyPressedUniform_AMC, 1);
	//	glUniform3f(ldUniform_AMC, 1.0f, 1.0f, 1.0f);
	//	glUniform3f(kdUniform_AMC, 0.5f, 0.5f, 0.5f);
	//	glUniform4f(lightPositionUniform_AMC, 0.0f, 0.0f, 2.0f, 1.0f);
	//}
	//else
	//{
	//	glUniform1i(keyPressedUniform_AMC, 0);
...
-(void)uninitialize
{
	...
	if (vbo_normals_cube_AMC)
	{
		glDeleteBuffers(1, &vbo_normals_cube_AMC);
		vbo_normals_cube_AMC = 0;
	}
