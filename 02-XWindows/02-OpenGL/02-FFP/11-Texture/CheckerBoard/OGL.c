...
void loadGLTexture(void)
{
    ...
    glGenTextures(1, &texture_checkerBoard); // creates empty texture
	...
	glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_NEAREST); //hite mathematically generate kartoy image madhun copy nahi karat tyamule color cha samdh nahi
	glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_S, GL_REPEAT); //wrapping X axis, jr ka amcha mathematical texture  object peksha motha zhal tr x coordinate la wrap kara, ani tya wrap kartan je pattern vaprtay te repeat kara
	glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_T, GL_REPEAT);

    // glu Build kr 2d mipmaps, cannot use in PP
    gluBuild2DMipmaps(GL_TEXTURE_2D, 3, CHECKERBOARD_WIDTH, CHECKERBOARD_HEIGHT, GL_RGBA, GL_UNSIGNED_BYTE, checkerBoard);
...
void display(void)
{
	...
	//Oblique Faced right-sided quad
	glTexCoord2f(1.0f, 1.0f);
