...
RenderState gState = STATE_WHITE;
...
int main(void)
{
    ...
    XStoreName(gpDisplay, window, "RTR2024-150 Omkar Kashid | Tweaked Smiley ");
...
void display(void)
{
    ...
    switch (gState)
    {
    case STATE_WHITE: /* plain white quad */
    ...
    case STATE_4X: // case 3
        glBindTexture(GL_TEXTURE_2D, texture_smiley);

        /* ?? top-left ??????????????????????????????????????????? */
        glBegin(GL_QUADS);
        glTexCoord2f(1.0f, 1.0f);
        glVertex3f(0.0f, 1.0f, 0.0f); /* upper-right */
        glTexCoord2f(0.0f, 1.0f);
        glVertex3f(-1.0f, 1.0f, 0.0f); /* upper-left  */
        glTexCoord2f(0.0f, 0.0f);
        glVertex3f(-1.0f, 0.0f, 0.0f); /* lower-left  */
