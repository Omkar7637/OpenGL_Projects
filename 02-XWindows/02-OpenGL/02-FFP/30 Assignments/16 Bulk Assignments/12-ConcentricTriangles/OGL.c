...
void display(void)
{
	...
	GLfloat vertex = 0.05f;
	for (int i = 0; i < 10; i++)
	{
		
		glColor3f(triangleColors[i][0], triangleColors[i][1], triangleColors[i][2]);
		glBegin(GL_LINE_LOOP);

			glVertex3f(0.0f, vertex, 0.0f);
			glVertex3f(-vertex, -vertex, 0.0f);
			glVertex3f(vertex, -vertex, 0.0f);

		glEnd();
		fprintf(gpFile, "Vertex=%f\n", vertex);
		vertex = vertex + 0.025f;
	}
