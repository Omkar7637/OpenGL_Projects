...
GLuint mvpMatrixUniform = 0; // modelview projection

// here we have broken down mvpMatrix into modelview and projection matrix
GLuint modelViewMatrixUniform = 0;	// model view matrix
GLuint projectionMatrixUniform = 0;	// projection view matrix 
...
GLfloat lightDifused[] = { 1.0f, 1.0f, 1.0f, 1.0f };    // RGBA
GLfloat materialDifused[] = { 0.5f, 0.5f, 0.5f, 1.0f }; // grey light
...
int initialize(void)
{
    ...
    const GLchar *vertexShaderSourceCode =
        ...
        "out_Difuse_Light = uLd*uKd*max(dot(lightSource,transformedNormal),0.0);\n" \
        "}\n" \
		"else\n" \
		"{\n" \
		"out_Difuse_Light = vec3(1.0, 1.0, 1.0);\n" \
        "}\n" \
        "}\n";
    ...
    */
    ...
    const GLchar *fragmentShaderSourceCode =
        "#version 460 core\n" \
        "in vec3 out_Difuse_Light;\n" \
        "out vec4 FragColor;\n" \
        "void main(void)\n" \
        "{\n" \
        "FragColor = vec4(out_Difuse_Light, 1.0f);\n" \
        "}\n";
...
void display(void)
{
	...
	mat4 modelViewMatrix = mat4::identity(); // this is analogous to glLoadIdentity() in display for modelview matrix
	...
	// Combine transformations: Tranlate -> Rotate -> Scale
	...
	if (bLight) {
		...
		glUniform3fv(ldUniform, 1, lightDifused);
		glUniform3fv(kdUniform, 1, materialDifused);
...
{
	// code
	angleCube = (angleCube - 0.03f) + speedAngle;
	if (angleCube <= 0.0f) {
