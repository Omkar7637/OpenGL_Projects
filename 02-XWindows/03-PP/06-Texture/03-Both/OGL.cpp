...
int initialize(void)
{
    ...
    */
    ...
    glBufferData(GL_ARRAY_BUFFER, sizeof(pyramid_texcord), pyramid_texcord, GL_STATIC_DRAW);
    glVertexAttribPointer(AMC_ATTRIBUTE_TEXCOORD, 2, GL_FLOAT, GL_FALSE, 0, NULL);
	...
	// Load Textures
	char textureFileKundali[] = "Vijay_Kundali.bmp";
	if(loadGLTexture(&textureKundali, textureFileKundali) == False)
	{
    ...
    {
        fprintf(gpFile, "Loaded texture %s -> id %u\n", textureFileKundali, textureKundali);
    }

    char textureFileStone[] = "Stone.bmp";
	if(loadGLTexture(&textureStone, textureFileStone) == False)
	{
...
void display(void)
{
    ...
    mat4 translationMatrix         = vmath::translate(1.5f, 0.0f, -6.0f);
