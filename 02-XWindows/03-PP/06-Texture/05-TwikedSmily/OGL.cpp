    ...
    */
    ...
    const GLchar *fragmentShaderSourceCode =
        ...
        "{\n" \
            ...
            "if(uKeyPressed != 1 && uKeyPressed != 2 && uKeyPressed != 3 && uKeyPressed != 4)\n" \
            "{\n" \
...
void display(void)
{
	...
	GLfloat rectangle_textCoords[8];

	if (iKeyPress == 1) {
		rectangle_textCoords[0] = 0.5f;
		rectangle_textCoords[1] = 0.5f;

		rectangle_textCoords[2] = 0.0f;
		rectangle_textCoords[3] = 0.5f;
		
		rectangle_textCoords[4] = 0.0f;
		rectangle_textCoords[5] = 0.0f;

		rectangle_textCoords[6] = 0.5f;
		rectangle_textCoords[7] = 0.0f;
		glUniform1i(keyPressUniform, 1);
	}
	else if (iKeyPress == 2) {
		rectangle_textCoords[0] = 1.0f;
		rectangle_textCoords[1] = 1.0f;

		rectangle_textCoords[2] = 0.0f;
		rectangle_textCoords[3] = 1.0f;

		rectangle_textCoords[4] = 0.0f;
		rectangle_textCoords[5] = 0.0f;

		rectangle_textCoords[6] = 1.0f;
		rectangle_textCoords[7] = 0.0f;
		glUniform1i(keyPressUniform, 2);
	}
	else if (iKeyPress == 3) {
		rectangle_textCoords[0] = 2.0f;
		rectangle_textCoords[1] = 2.0f;

		rectangle_textCoords[2] = 0.0f;
		rectangle_textCoords[3] = 2.0f;

		rectangle_textCoords[4] = 0.0f;
		rectangle_textCoords[5] = 0.0f;

		rectangle_textCoords[6] = 2.0f;
		rectangle_textCoords[7] = 0.0f;
		...
		rectangle_textCoords[2] = 0.5f;
		rectangle_textCoords[3] = 0.5f;

		rectangle_textCoords[4] = 0.5f;
		rectangle_textCoords[5] = 0.5f;

		rectangle_textCoords[6] = 0.5f;
		rectangle_textCoords[7] = 0.5f;
		glUniform1i(keyPressUniform, 4);
	}
	else {
		rectangle_textCoords[0] = 0.0f;
		rectangle_textCoords[1] = 0.0f;

		rectangle_textCoords[2] = 0.0f;
		rectangle_textCoords[3] = 0.0f;

		rectangle_textCoords[4] = 0.0f;
		rectangle_textCoords[5] = 0.0f;

		rectangle_textCoords[6] = 0.0f;
		rectangle_textCoords[7] = 0.0f;
		glUniform1i(keyPressUniform, 0);
	}

	// step 2 : Binding point(Target point) first comes 0 than 1 then 2
	glBindBuffer(GL_ARRAY_BUFFER, vbo_texcoord_smiley);

	// step 3 : size of the data & it will be static (will send once)
	glBufferData(GL_ARRAY_BUFFER, sizeof(rectangle_textCoords), rectangle_textCoords, GL_DYNAMIC_DRAW); // we will be passing array in display based on inputs

	// step 4 : pass the data
	glVertexAttribPointer(AMC_ATTRIBUTE_TEXCOORD,	// where to bind shader
		2,											// use elements of 3 groups each - vertex3f
		GL_FLOAT,									// data type (array data type)
		GL_FALSE,									// not Normalised then GL_FALSE
		0,											// when passing multiple attributes
		NULL										// buffer array pointer if using previous parameteres that's why null
	);

	// step 5 : 
	glEnableVertexAttribArray(AMC_ATTRIBUTE_TEXCOORD);

	// step 6 : unbind buffer
	glBindBuffer(GL_ARRAY_BUFFER, 0);
