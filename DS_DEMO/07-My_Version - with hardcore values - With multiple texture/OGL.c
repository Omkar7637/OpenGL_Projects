...
#include"OGL.h"
// #include"O_Rabbit.h"
#include"Sketchfab_2022_08_23_04_23_33.h"
...
GLuint textureIDs[4]; // adjust size to number of materials
...
{	
	...
	loadGLTexture(&textureIDs[0], MAKEINTRESOURCE(IDBITMAP_T1));
    loadGLTexture(&textureIDs[1], MAKEINTRESOURCE(IDBITMAP_T2));
    loadGLTexture(&textureIDs[2], MAKEINTRESOURCE(IDBITMAP_T3));
    loadGLTexture(&textureIDs[3], MAKEINTRESOURCE(IDBITMAP_T4));


	// Model rabbit  = loadOBJModel("O_Rabbit.obj"); // or however you're loading
	// loadGLTexture(&rabbit.textureID, MAKEINTRESOURCE(IDBITMAP_TERRAIN));
	// rabbit.posX = 0.0f;  rabbit.posY = 0.0f; rabbit.posZ = 0.0f;
	// rabbit.rotY = 0.0f;
	// rabbit.scaleX = rabbit.scaleY = rabbit.scaleZ = 200.0f;
