...
float speed = 2.0f; // For the walking Speed
...
Model loadOBJModel(const char *filename) {
    ...
    while (fgets(line, sizeof(line), fp)) {
        ...
        else if (strncmp(line, "vt ", 3) == 0) texCoordCount++;
    ...
    TexCoord *texCoords = malloc(texCoordCount * sizeof(TexCoord));
    Normal *normals = malloc(normalCount * sizeof(Normal));
    ...
    while (fgets(line, sizeof(line), fp)) {
        if (strncmp(line, "v ", 2) == 0) {
            ...
            sscanf(line, "vt %f %f", &texCoords[vtIndex].u, &texCoords[vtIndex].v);
            vtIndex++;
        } else if (strncmp(line, "vn ", 3) == 0) {
            sscanf(line, "vn %f %f %f", &normals[vnIndex].nx, &normals[vnIndex].ny, &normals[vnIndex].nz);
            vnIndex++;
        } else if (strncmp(line, "f ", 2) == 0) {
            sscanf(line, "f %d/%d/%d %d/%d/%d %d/%d/%d",
                   &faces[fIndex].v[0], &faces[fIndex].vt[0], &faces[fIndex].vn[0],
                   &faces[fIndex].v[1], &faces[fIndex].vt[1], &faces[fIndex].vn[1],
                   &faces[fIndex].v[2], &faces[fIndex].vt[2], &faces[fIndex].vn[2]);
            fIndex++;
        }
    }
    fclose(fp);
    ...
    Model model;
    model.totalVertices = faceCount * 3;
    model.vertexArray = malloc(model.totalVertices * 3 * sizeof(float));
    model.normalArray = malloc(model.totalVertices * 3 * sizeof(float));
    model.texCoordArray = malloc(model.totalVertices * 2 * sizeof(float));

    int k = 0, n = 0, t = 0;
    for (int i = 0; i < faceCount; i++) {
        for (int j = 0; j < 3; j++) {
            int vi = faces[i].v[j] - 1;
            int vni = faces[i].vn[j] - 1;
            int vti = faces[i].vt[j] - 1;

            model.vertexArray[k++] = vertices[vi].x;
            model.vertexArray[k++] = vertices[vi].y;
            model.vertexArray[k++] = vertices[vi].z;

            model.normalArray[n++] = normals[vni].nx;
            model.normalArray[n++] = normals[vni].ny;
            model.normalArray[n++] = normals[vni].nz;

            model.texCoordArray[t++] = texCoords[vti].u;
            model.texCoordArray[t++] = texCoords[vti].v;
        }
    }
    ...
    // glBufferData(GL_ARRAY_BUFFER, model.totalVertices * 3 * sizeof(float), model.vertexArray, GL_STATIC_DRAW);

    // glGenBuffers(1, &model.vboNormals);
    // glBindBuffer(GL_ARRAY_BUFFER, model.vboNormals);
    // glBufferData(GL_ARRAY_BUFFER, model.totalVertices * 3 * sizeof(float), model.normalArray, GL_STATIC_DRAW);

    // glGenBuffers(1, &model.vboTexCoords);
    // glBindBuffer(GL_ARRAY_BUFFER, model.vboTexCoords);
    // glBufferData(GL_ARRAY_BUFFER, model.totalVertices * 2 * sizeof(float), model.texCoordArray, GL_STATIC_DRAW);
...
// EXAMPLE STAGE 1 MODEL LOADING USING GLEW.
// void renderModel(Model *model) {
...
//     // glDrawArrays(GL_TRIANGLES, 0, model->totalVertices);
...
// }
    ...
    models[modelCount++] = loadOBJModel("Cube.obj");
    // models[modelCount++] = loadOBJModel("Sphere.obj");
...
void display(void)
{
    ...
    renderModel(&models[0]);  // First model
    glPopMatrix();

    if (modelCount > 1) {
        glPushMatrix();
        glTranslatef(1.5, 0, 0);
        renderModel(&models[1]);  // Second model
        glPopMatrix();
    }
	// //==================
