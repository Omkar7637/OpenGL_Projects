        ...
        for (const auto& prim : mesh.primitives)
        {
            ...
            if (prim.attributes.count("NORMAL"))
            {
                ...
                normals = reinterpret_cast<const float*>(
                    &model.buffers[nView.buffer].data[
            ...
            if (prim.material >= 0)
            {
                ...
                {
                    m.alpha = (float)mat.pbrMetallicRoughness.baseColorFactor[3];
                }
...
void ModelLoader::Update(float time)
{
    ...
    for (size_t i = 0; i < anim->model.nodes.size(); i++)
    {
        animatedNodeMatrix *= GetNodeMatrix(anim->model.nodes[i]);
    }
