...
public class GLESView extends GLSurfaceView implements GLSurfaceView.Renderer, OnDoubleTapListener, OnGestureListener
{
	...
	private void display()
	{
		...
		Matrix.rotateM(modelViewMatrix, 0, angle_rectangle, 0.0f, 1.0f, 0.0f);
		Matrix.multiplyMM(modelViewProjectionMatrix, 0, perspectiveProjectionMatrix, 0, modelViewMatrix, 0);
	...
	{
		// Code
		angle_triangle = angle_triangle + 0.3f;
		if (angle_triangle >= 360.0f)
		{
			angle_triangle = angle_triangle - 360.0f;
		}

		angle_rectangle = angle_rectangle - 0.3f;
		if (angle_rectangle <= 0.0f)
		{
			angle_rectangle = angle_rectangle + 360.0f;
		}
	}
