...
public class GLESView extends GLSurfaceView implements GLSurfaceView.Renderer, OnDoubleTapListener, OnGestureListener
{
	...
	private void initialize(GL10 gl)
	{
		...
		GLES32.glBufferData(GLES32.GL_ARRAY_BUFFER, sqaure_texcoord.length * 4, textureBuffer , GLES32.GL_STATIC_DRAW);
		...
		texImage[0] = loadGLTexture();		// kundali, stone
		// about texture image
	...
	private int loadGLTexture()
	{
		...
		// create the Bitmap from data array given using above function
		...
		GLES32.glTexParameteri(GLES32.GL_TEXTURE_2D, GLES32.GL_TEXTURE_WRAP_S, GLES32.GL_REPEAT);	// for chk
	...
	{
		float[] sqaure_position = new float[12];
		...
		sqaure_position[0] = 0.0f;
		sqaure_position[1] = 1.0f;
		sqaure_position[2] = 0.0f;
		sqaure_position[3] = -2.0f;
		sqaure_position[4] = 1.0f;
		sqaure_position[5] = 0.0f;
		sqaure_position[6] = -2.0f;
		sqaure_position[7] = -1.0f;
		sqaure_position[8] = 0.0f;
		sqaure_position[9] = 0.0f;
		sqaure_position[10] = -1.0f;
		sqaure_position[11] = 0.0f;
		...
		GLES32.glBufferData(GLES32.GL_ARRAY_BUFFER, sqaure_position.length * 4, positionBuffer, GLES32.GL_DYNAMIC_DRAW);
		...
		sqaure_position[0] = 2.4142f;
		sqaure_position[1] = 1.0f;
		sqaure_position[2] = -1.41421f;
		sqaure_position[3] = 1.0f;
		sqaure_position[4] = 1.0f;
		sqaure_position[5] = 0.0f;
		sqaure_position[6] = 1.0f;
		sqaure_position[7] = -1.0f;
		sqaure_position[8] = 0.0f;
		sqaure_position[9] = 2.4142f;
		sqaure_position[10] = -1.0f;
		sqaure_position[11] = -1.41421f;

		GLES32.glBindBuffer(GLES32.GL_ARRAY_BUFFER, vbo_position[0]);

		byteBuffer = ByteBuffer.allocateDirect(sqaure_position.length * 4);
		byteBuffer.order(ByteOrder.nativeOrder());
	...
	private void uninitialize()
	{
		...
		if (texImage[0] > 0)
		{
			GLES32.glDeleteTextures(1, texImage, 0);
			texImage[0] = 0;
		}
		if (vbo_texture[0] > 0)
		{
