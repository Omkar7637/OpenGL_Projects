...
void display(void)
{
    ...
    float modelScale = 0.5f;   // try 0.1f, 1.0f, 2.0f
	...
	// FIX GLTF ORIENTATION (ONE TIME)
    // GLTF is Y-up → OpenGL camera expects Z-forward
    mat4 fixRotation = vmath::rotate(
        vmath::radians(-90.0f),   // TRY -90 or +90
        1.0f, 0.0f, 0.0f
    );

    // ROTATION (animated)
    float rotationSpeed = 0.0f; // degrees per second
    float angle = vmath::radians(animTime * rotationSpeed);

    // TRANSLATION (position in world)
    mat4 translateMatrix = vmath::translate(
        0.0f, 0.0f, 1.0f
    );

    // FINAL MODEL MATRIX (IMPORTANT ORDER)
    mat4 modelMatrix =
        translateMatrix *
		fixRotation *
        scaleMatrix;
    ...
    // Texture unit (GLTF loader binds internally)
    glUniform1i(textureSamplerUniform, 0);
