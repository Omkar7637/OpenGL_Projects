...
int initialized(void)
{
	...
	orthoMatrix[0] = 2.0f / (r - l);
	orthoMatrix[1] = 0.0f;
	orthoMatrix[2] = 0.0f;
	orthoMatrix[3] = 0.0f;
	orthoMatrix[4] = 0.0f;
	orthoMatrix[5] = 2.0f / (t - b);
	orthoMatrix[6] = 0.0f;
	orthoMatrix[7] = 0.0f;
	orthoMatrix[8] = 0.0f;
	orthoMatrix[9] = 0.0f;
	orthoMatrix[10] = -2.0f / (f - n);
	orthoMatrix[11] = 0.0f;
	orthoMatrix[12] = -(r + l) / (r - l);
	orthoMatrix[13] = -(t + b) / (t - b);
	orthoMatrix[14] = -(f + n) / (f - n);
	orthoMatrix[15] = 1.0f;
...
void resize(int width, int height)
{
	...
	if (width <= height)
	{
		b = b * ((GLfloat)height / (GLfloat)width);
		t = t * ((GLfloat)height / (GLfloat)width);
		glMultMatrixf(orthoMatrix);
	}
	else
	{
		l = l * ((GLfloat)height / (GLfloat)width);
		r = r * ((GLfloat)height / (GLfloat)width);
		glMultMatrixf(orthoMatrix);
	}
}
...
void display(void)
{
	...
	//glLoadIdentity();	// doing manually
	glLoadMatrixf(identityMatrix);

	glBegin(GL_TRIANGLES);

	glColor3f(1.0f, 0.0f, 0.0f);
	glVertex3f(0.0f, 50.0f, 0.0f);							// size changed from 1.0f to 50.0f  
