...
void display(void)
{
	...
	for (int i = 0; i < 10; i++)
	{
		glColor3fv(colors[i]); // SetColors

		glLineWidth(2.5f); // Width of lines

		glBegin(GL_LINE_LOOP);

		glVertex3f(0.0f, size, 0.0f); // TOP VERTEX
		glVertex3f(-size, -size, 0.0f); // BOTTOM LEFT VERTEX
		glVertex3f(size, -size, 0.0f);// BOTTOM RIGHT VERTEX

		glEnd();

		size -= 0.08f; // SIZE OF NEXT TRIANGLE
	}
	//Swap the buffers
