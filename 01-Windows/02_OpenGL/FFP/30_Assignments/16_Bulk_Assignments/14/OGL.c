...
HGLRC ghrc = NULL; 
int segments = 1000;     // Number of segments for smoothness
int count = 0;
float gradius = 0.0f;
...
void display(void)
{
	...
	GLfloat radius = 0.1f;  // Initial radius

	for (int i = 0; i < 10; i++)
	{
		glColor3fv(colors[i%10]); // Set color
		glLineWidth(2.5f); // Line thickness

		glBegin(GL_LINE_LOOP);
		for (int j = 0; j < segments; j++)
		{
			float angle = 2.0f * M_PI * j / segments; // Angle in radians
		...
		radius += 0.05f + gradius; // Shrink radius for next circle
	}
	//Swap the buffers
