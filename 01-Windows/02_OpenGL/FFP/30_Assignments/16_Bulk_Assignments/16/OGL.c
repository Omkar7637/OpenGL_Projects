...
float trianglePosX = -2.0f;
float trianglePosY = -2.0f;
float triangleAngle = 0.0f;

float circlePosX = 2.0f;
float circlePosY = -2.0f;
...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpszCmdLine, int iCmdShow)
{
	...
	PlaySound(TEXT("HarryPotter_Theme.wav"), NULL, SND_ASYNC | SND_LOOP);
...
void display(void)
{
	...
	glColor3f(1.0f, 0.84f, 0.0f); // Yellow Gold


	glColor3f(1.0f, 0.84f, 0.0f); // Yellowish Gold Color

	// Triangle vertices
	float Ax = 0.0f, Ay = 0.5f;
	float Bx = -0.5f, By = -0.5f;
	float Cx = 0.5f, Cy = -0.5f;

	// Draw large hollow triangle
	glTranslatef(trianglePosX, trianglePosY, 0.0f);
	...
	float c = sqrt((Ax - Bx) * (Ax - Bx) + (Ay - By) * (Ay - By));

	// Compute incenter
	float Ix = (a * Ax + b * Bx + c * Cx) / (a + b + c);
	float Iy = (a * Ay + b * By + c * Cy) / (a + b + c);
	...
	if (triangleDone)
	{
		glLoadIdentity();
		glTranslatef(0.0f, 0.0f, -8.0f);
		glTranslatef(circlePosX, circlePosY, 0.0f);
		...
		for (int i = 0; i < segments; i++) {
			...
			float x = inradius * cos(angle) + Ix;
			float y = inradius * sin(angle) + Iy;
			glVertex3f(x, y, 0.0f);
		}
		glEnd();
	}
...
void update(void)
{
	...
	if (!triangleDone)
	{
		trianglePosX += 0.002f;
		trianglePosY += 0.002f;
		if (trianglePosX >= 0.0f && trianglePosY >= 0.0f)
		{
			triangleDone = TRUE;
		}
	}
	else if (!circleDone)
	{
		...
		if (circlePosX <= 0.0f && circlePosY >= 0.0f)
		{
			circlePosX = 0.0f;
			circlePosY = 0.0f;
			circleDone = TRUE;
