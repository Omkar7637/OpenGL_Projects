...
LRESULT CALLBACK WndProc(HWND hwnd, UINT iMsg, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_CHAR:
		...
		case 'D':
			//date = (date + 6) % 360; //360 degree = 1 revolution so date ?
			break;
		case 'd':
			//date = (date - 6) % 360;
			break;
...
int initialized(void)
{
	...
	// intialize quadric
	quadric = gluNewQuadric(); //giving memory
...
void display(void)
{
	...
	// Save above transformation in to modelview matrix stack
	glPushMatrix();
	...
	// Draw sphere
	gluSphere(quadric, 0.75f, 30, 30); // gloabal variable, radius, horizontal subdivision, vertical subdivision(segments)

	// After drawing sphere we need to go back to original to draw earth hence restore the matrix
	glPopMatrix();

	// code for earth
	// Again save this matrix to let us to come bac again
	glPushMatrix();

	// Let the earth revolve around the sun
	glRotatef((GLfloat)year, 0.0f, 1.0f, 0.0f);

	// Keep earth is at some distance from the sun
