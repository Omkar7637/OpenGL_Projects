...
int initialized(void)
{
	...
	glGenVertexArrays(1, &vao_pyramid); // vao_pyramid vertex array object
	...
	glBindBuffer(GL_ARRAY_BUFFER, 0);
	glBindVertexArray(0); // vao_pyramid unbind
...
void display(void)
{
	...
	else if (iKeyPressed == 4)
	{
		...
		rectangle_texcoords[2] = 0.5f;
		rectangle_texcoords[3] = 0.5f;

		rectangle_texcoords[4] = 0.5f;
		rectangle_texcoords[5] = 0.5f;

		rectangle_texcoords[6] = 0.5f;
		rectangle_texcoords[7] = 0.5f;
		glUniform1i(keyPressUniform, 4);
	}
	else
	{
		rectangle_texcoords[0] = 0.0f;
		rectangle_texcoords[1] = 0.0f;

		rectangle_texcoords[2] = 0.0f;
		rectangle_texcoords[3] = 0.0f;

		rectangle_texcoords[4] = 0.0f;
		rectangle_texcoords[5] = 0.0f;

		rectangle_texcoords[6] = 0.0f;
		rectangle_texcoords[7] = 0.0f;
		glUniform1i(keyPressUniform, 0);
	}

	//Step 2: Binding point(Target point) first comes 0 then 1 then 2
	glBindBuffer(GL_ARRAY_BUFFER, vbo_TexCoord_pyramid);
	//Step 3: size of data & data & it will be static (will send once)
	glBufferData(GL_ARRAY_BUFFER, sizeof(rectangle_texcoords), rectangle_texcoords, GL_DYNAMIC_DRAW);	// We will be passing array in display based on inputs.
	//Step 4: pass the data
	glVertexAttribPointer(AMC_ATTRIBUTE_TEXCOORD,			// where to bind in shader (khidki to khidki)
		2,													// use elements of 3 groups each - vertex3f
		GL_FLOAT,											// data type (array data type)
		GL_FALSE,											// not Normalized then GL_FALSE
		0,													// when passing multiple attributes - dhanga(strides) kitichya taaku. will see in interleaved.
		NULL												// Buffer array pointer if using previous parameters that's why NULL.
	);
	//Step 5:
	glEnableVertexAttribArray(AMC_ATTRIBUTE_TEXCOORD);
	...
	glDrawArrays(GL_TRIANGLE_FAN, 0, 4); //saying go to gpu to start drawing
	//unbind
	glBindTexture(GL_TEXTURE_2D, 0);
