...
int initialized(void)
{
	...
	if(status == GL_FALSE)
	{
		glGetShaderiv(geometryShaderObject, GL_INFO_LOG_LENGTH, &infoLogLenth);
		...
		if(infoLogLenth > 0)
		{
			...
			if(szinfoLog != NULL)
			{
				glGetShaderInfoLog(geometryShaderObject, infoLogLenth, NULL, szinfoLog);
	...
	// get the location form uniformed shader
