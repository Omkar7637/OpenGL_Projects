...
// Tesellation realted gloabal variables
GLuint numberofLinesSegmentUniform;
GLuint numberofLineStripUniform;
...
int initialized(void)
{
	...
	"uniform int unumberoflinestrip;\n" \
	"void main(void)\n" \
	"{\n" \
	"	gl_out[gl_InvocationID].gl_Position = gl_in[gl_InvocationID].gl_Position;\n" \
	"	gl_TessLevelOuter[0] = float(unumberoflinestrip);\n" \
	...
	if(status == GL_FALSE)
	{
		glGetShaderiv(tessellationControlShaderObject, GL_INFO_LOG_LENGTH, &infoLogLenth);
		...
		if(infoLogLenth > 0)
		{
			...
			if(szinfoLog != NULL)
			{
				glGetShaderInfoLog(tessellationControlShaderObject, infoLogLenth, NULL, szinfoLog);
				fprintf(gpFile, "TESS CONTROL SHADER COMPILATION LOG = %s\n", szinfoLog);
				free(szinfoLog);
				szinfoLog = NULL;
			}
		}
	...
	if(status == GL_FALSE)
	{
		glGetShaderiv(tessellationEvaluationShaderObject, GL_INFO_LOG_LENGTH, &infoLogLenth);
		...
		if(infoLogLenth > 0)
		{
			...
			if(szinfoLog != NULL)
			{
				glGetShaderInfoLog(tessellationEvaluationShaderObject, infoLogLenth, NULL, szinfoLog);
				fprintf(gpFile, "TESS EVALUATION SHADER COMPILATION LOG = %s\n", szinfoLog);
				free(szinfoLog);
				szinfoLog = NULL;
			}
		}
	...
	numberofLinesSegmentUniform = glGetUniformLocation(shaderProgramObject, "unumberoflinesegments");
	numberofLineStripUniform = glGetUniformLocation(shaderProgramObject, "unumberoflinestrip");
	lineColorUniform = glGetUniformLocation(shaderProgramObject, "ulineColor");

	// Provide vartex position, color, normal, texcod, etc,.
	const GLfloat line_position[] = 
	{
		-1.0f, -1.0f,
		-0.5f,  1.0f,
		0.5f, -1.0f,
...
void display(void)
{
	...
	// send number of line segments to tessellation control shader
	glUniform1i(numberofLinesSegmentUniform, uiNumberOfLineSegments);

	TCHAR str[255];
	wsprintf(str, TEXT("Omkar Ankush Kashid : (Number of Line Segments = %d)"), uiNumberOfLineSegments);
	SetWindowText(ghwnd, str);

	// send number of line strip to tessellation control shader
	glUniform1i(numberofLineStripUniform, 1);
	// send line color to fragment shader
	glUniform4fv(lineColorUniform, 1, vmath::vec4(1.0f, 1.0f, 0.0f, 1.0f)); // yellow color

	// bind with vao
	glBindVertexArray(vao);

	// decide how many vertices for one patch
	glPatchParameteri(GL_PATCH_VERTICES, 4); // 4 control points for bezier	
